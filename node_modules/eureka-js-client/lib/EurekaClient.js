...
var Eureka = function (_EventEmitter) {
  ...
  */
  Eureka.prototype.buildEurekaUrl = function buildEurekaUrl() {
    ...
    this.lookupCurrentEurekaHost(function (err, eurekaHost) {
      if (err) return callback(err);
      var _config$eureka = _this2.config.eureka;
  ...
  */
  ...
  Eureka.prototype.start = function start() {
    ...
    (0, _async.series)([function (done) {
      if (_this3.metadataClient && _this3.config.eureka.fetchMetadata) {
        return _this3.addInstanceMetadata(done);
      }
      done();
    }, function (done) {
      _this3.register(done);
    }, function (done) {
      _this3.startHeartbeats();
      if (_this3.config.eureka.fetchRegistry) {
        _this3.startRegistryFetches();
        if (_this3.config.eureka.waitForRegistry) {
          var _ret = function () {
            var waitForRegistryUpdate = function waitForRegistryUpdate(cb) {
              _this3.fetchRegistry(function () {
                var found = _this3.getInstancesByVipAddress(_this3.config.instance.vipAddress);
      ...
      if (err) _this3.logger.warn('Error starting the Eureka Client', err);
      _this3.emit('started');
  ...
  */
  ...
  Eureka.prototype.register = function register() {
    ...
    var connectionTimeout = setTimeout(function () {
      _this4.logger.warn('It looks like it\'s taking a while to register with ' + 'Eureka. This usually means there is an issue connecting to the host ' + 'specified. Start application with NODE_DEBUG=request for more logging.');
    }, 10000);
    this.buildEurekaUrl(function (err, eurekaUrl) {
      if (err) return callback(err);
      _request2.default.post({
        url: eurekaUrl + _this4.config.instance.app,
        json: true,
        body: { instance: _this4.config.instance },
        gzip: true
      }, function (error, response, body) {
        clearTimeout(connectionTimeout);
        if (!error && response.statusCode === 204) {
          _this4.logger.info('registered with eureka: ', _this4.config.instance.app + '/' + _this4.instanceId);
          _this4.emit('registered');
          return callback(null);
        } else if (error) {
          _this4.logger.warn('Error registering with eureka client.', error);
  ...
  Eureka.prototype.deregister = function deregister() {
    ...
    this.buildEurekaUrl(function (err, eurekaUrl) {
      ...
      _request2.default.del({
        url: '' + eurekaUrl + _this5.config.instance.app + '/' + _this5.instanceId,
        gzip: true
      }, function (error, response, body) {
        if (!error && response.statusCode === 200) {
          _this5.logger.info('de-registered with eureka: ', _this5.config.instance.app + '/' + _this5.instanceId);
          _this5.emit('deregistered');
          return callback(null);
        } else if (error) {
          _this5.logger.warn('Error deregistering with eureka', error);
          return callback(error);
        }
  ...
  */
  ...
  Eureka.prototype.renew = function renew() {
    ...
    this.buildEurekaUrl(function (err, eurekaUrl) {
      if (err) {
        _this7.logger.warn('eureka heartbeat FAILED, will retry', err);
        return;
      }
      _request2.default.put({
        url: '' + eurekaUrl + _this7.config.instance.app + '/' + _this7.instanceId,
        gzip: true
      }, function (error, response, body) {
        if (!error && response.statusCode === 200) {
          _this7.logger.debug('eureka heartbeat success');
          _this7.emit('heartbeat');
        } else if (!error && response.statusCode === 404) {
          _this7.logger.warn('eureka heartbeat FAILED, Re-registering app');
          _this7.register();
        } else {
          if (error) {
            _this7.logger.error('An error in the request occured.', error);
          }
          _this7.logger.warn('eureka heartbeat FAILED, will retry.', 'status: ' + (response ? response.statusCode : 'unknown') + ' body: ' + body);
        }
      });
    });
  };
  ...
  */
  ...
  Eureka.prototype.startRegistryFetches = function startRegistryFetches() {
    ...
    this.registryFetch = setInterval(function () {
      _this8.fetchRegistry(function (err) {
        if (err) _this8.logger.warn('Error fetching registries', err);
      });
  ...
  Eureka.prototype.fetchRegistry = function fetchRegistry() {
    ...
    this.buildEurekaUrl(function (err, eurekaUrl) {
      ...
      _request2.default.get({
        ...
        if (!error && response.statusCode === 200) {
          _this9.logger.debug('retrieved registry successfully');
          try {
            _this9.transformRegistry(JSON.parse(body));
          } catch (ex) {
            return callback(ex);
          }
          _this9.emit('registryUpdated');
          return callback(null);
        } else if (error) {
          _this9.logger.warn('Error fetching registry', error);
          return callback(error);
        }
    ...
    if (!registry) {
      ...
      var _ret2 = function () {
        ...
        if (Array.isArray(registry.applications.application)) {
          ...
          _this10.transformApp(registry.applications.application, newCache);
        }
        _this10.cache = newCache;
      }();
  ...
  Eureka.prototype.transformApp = function transformApp(app, cache) {
    ...
    if (app.instance.length) {
      var instances = app.instance.filter(function (instance) {
        return _this11.validateInstance(instance);
      });
  ...
  */
  ...
  Eureka.prototype.addInstanceMetadata = function addInstanceMetadata() {
    ...
    this.metadataClient.fetchMetadata(function (metadataResult) {
      _this12.config.instance.dataCenterInfo.metadata = (0, _merge2.default)(_this12.config.instance.dataCenterInfo.metadata, metadataResult);
      var useLocal = _this12.config.eureka.useLocalMetadata;
      var metadataHostName = metadataResult[useLocal ? 'local-hostname' : 'public-hostname'];
      _this12.config.instance.hostName = metadataHostName;
      _this12.config.instance.ipAddr = metadataResult[useLocal ? 'local-ipv4' : 'public-ipv4'];

      if (_this12.config.instance.statusPageUrl) {
        var statusPageUrl = _this12.config.instance.statusPageUrl;

        var replacedUrl = statusPageUrl.replace('__HOST__', metadataHostName);
        _this12.config.instance.statusPageUrl = replacedUrl;
      }
      if (_this12.config.instance.healthCheckUrl) {
        var healthCheckUrl = _this12.config.instance.healthCheckUrl;

        var _replacedUrl = healthCheckUrl.replace('__HOST__', metadataHostName);
        _this12.config.instance.healthCheckUrl = _replacedUrl;
      }
  ...
  Eureka.prototype.lookupCurrentEurekaHost = function lookupCurrentEurekaHost() {
    ...
    if (this.config.eureka.useDns) {
      this.locateEurekaHostUsingDns(function (err, resolvedHost) {
        return callback(err, resolvedHost);
      });
    } else {
