...
<html lang="en">
<head>
    <title>Code coverage report for src/eureka-client.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
  <div class='pad1'>
    <h1>
      <a href="../index.html">all files</a> / <a href="index.html">src/</a> eureka-client.js
    </h1>
    <div class='clearfix'>
      <div class='fl pad1y space-right2'>
        <span class="strong">93.95% </span>
        <span class="quiet">Statements</span>
        <span class='fraction'>202/215</span>
      </div>
      <div class='fl pad1y space-right2'>
        <span class="strong">82.54% </span>
        <span class="quiet">Branches</span>
        <span class='fraction'>104/126</span>
      </div>
      <div class='fl pad1y space-right2'>
      ...
      <div class='fl pad1y space-right2'>
        ...
        <span class='fraction'>142/155</span>
      </div>
      <div class='fl pad1y'>
...
375
376
377</td><td class="line-coverage quiet"><span class="cline-any cline-yes">1×</span>
...
&nbsp;
const noop = <span class="fstat-no" title="function not covered" >() =&gt; {</span>};
&nbsp;
/*
  Eureka JS client
...
&nbsp;
  ...
  */
  buildEurekaUrl(callback = <span class="branch-0 cbranch-no" title="branch not covered" >noop)</span> {
    this.lookupCurrentEurekaHost(eurekaHost =&gt; {
      callback(`${this.config.eureka.ssl ? <span class="branch-0 cbranch-no" title="branch not covered" >'https' </span>: 'http'}://${eurekaHost}:${this.config.eureka.port}${this.config.eureka.servicePath}`);
    });
  }
&nbsp;
  /*
  ...
  */
  start(callback = <span class="branch-0 cbranch-no" title="branch not covered" >noop)</span> {
    series([
      done =&gt; {
        if (<span class="missing-if-branch" title="if path not taken" >I</span>this.metadataClient &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >this.config.eureka.fetchMetadata)</span> {
          return <span class="cstat-no" title="statement not covered" >this.addInstanceMetadata(done);</span>
        }
        done();
      },
      done =&gt; {
        ...
        if (<span class="missing-if-branch" title="else path not taken" >E</span>this.config.eureka.fetchRegistry) {
          return this.fetchRegistry(done);
        }
        done<span class="cstat-no" title="statement not covered" >();</span>
      }
    ], callback);
  }
&nbsp;
  ...
  */
  stop(callback = <span class="branch-0 cbranch-no" title="branch not covered" >noop)</span> {
...
&nbsp;
  ...
  */
  register(callback = <span class="branch-0 cbranch-no" title="branch not covered" >noop)</span> {
...
&nbsp;
  ...
  */
  deregister(callback = <span class="branch-0 cbranch-no" title="branch not covered" >noop)</span> {
    this.buildEurekaUrl(eurekaUrl =&gt; {
      request.del({
...
&nbsp;
  ...
  */
  startHeartbeats<span class="fstat-no" title="function not covered" >() {</span>
    this<span class="cstat-no" title="statement not covered" >.heartbeat = setInterval(<span class="fstat-no" title="function not covered" >() =&gt; {</span></span>
      this.<span class="cstat-no" title="statement not covered" >renew();</span>
  ...
  renew() {
    this.buildEurekaUrl(eurekaUrl =&gt; {
      request.put({
        ...
        if (<span class="missing-if-branch" title="else path not taken" >E</span>!error &amp;&amp; response.statusCode === 200) {
          this.logger.debug('eureka heartbeat success');
        } else {
          if (<span class="cstat-no" title="statement not covered" >error) {</span>
            this.<span class="cstat-no" title="statement not covered" >logger.error('An error in the request occured.', error);</span>
          }
          this.<span class="cstat-no" title="statement not covered" >logger.warn('eureka heartbeat FAILED, will retry.', `status: ${response.statusCode} body: ${body}`);</span>
        }
      });
    });
  }
&nbsp;
  ...
  */
  startRegistryFetches<span class="fstat-no" title="function not covered" >() {</span>
    this<span class="cstat-no" title="statement not covered" >.registryFetch = setInterval(<span class="fstat-no" title="function not covered" >() =&gt; {</span></span>
      this.<span class="cstat-no" title="statement not covered" >fetchRegistry();</span>
...
&nbsp;
   ...
   */
  fetchRegistry(callback = <span class="branch-0 cbranch-no" title="branch not covered" >noop)</span> {
    this.buildEurekaUrl(eurekaUrl =&gt; {
      request.get({
        url: eurekaUrl,
        headers: {
...
&nbsp;
  ...
  */
  addInstanceMetadata(callback = <span class="branch-0 cbranch-no" title="branch not covered" >noop)</span> {
...
&nbsp;
      if (<span class="missing-if-branch" title="else path not taken" >E</span>this.config.instance.statusPageUrl) {
        this.config.instance.statusPageUrl = this.config.instance.statusPageUrl.replace('__HOST__', metadataResult['public-hostname']);
      }
      if (<span class="missing-if-branch" title="else path not taken" >E</span>this.config.instance.healthCheckUrl) {
...
&nbsp;
  ...
  */
  lookupCurrentEurekaHost(callback = <span class="branch-0 cbranch-no" title="branch not covered" >noop)</span> {
    if (<span class="missing-if-branch" title="if path not taken" >I</span>this.amazonDataCenter &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >this.config.eureka.useDns)</span> {
      this<span class="cstat-no" title="statement not covered" >.locateEurekaHostUsingDns(<span class="fstat-no" title="function not covered" >resolvedHost =&gt; {</span></span>
        return <span class="cstat-no" title="statement not covered" >callback(resolvedHost);</span>
      });
    } else {
  ...
  locateEurekaHostUsingDns(callback = noop) {
    ...
    dns.resolveTxt('txt.' + this.config.eureka.ec2Region + '.' + this.config.eureka.host, (error, addresses) =&gt; {
      if (<span class="missing-if-branch" title="if path not taken" >I</span>error) {
        throw <span class="cstat-no" title="statement not covered" >new Error('Error resolving eureka server list for region [' + this.config.eureka.ec2Region + '] using DNS: [' + error + ']');</span>
      }
      dns.resolveTxt('txt.' + addresses[0][Math.floor(Math.random() * addresses[0].length)], (error, results) =&gt; {
        if (<span class="missing-if-branch" title="if path not taken" >I</span>error) {
          throw <span class="cstat-no" title="statement not covered" >new Error('Error locating eureka server using DNS: [' + error + ']');</span>
        }
...
<div class='footer quiet pad2 space-top1 center small'>
  Code coverage
  generated by <a href="http://istanbul-js.org/" target="_blank">istanbul</a> at Tue Jan 19 2016 17:26:09 GMT-0800 (PST)
</div>
</div>
